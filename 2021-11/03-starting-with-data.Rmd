...
plots in our study area. The dataset is stored as a comma separated value (CSV) file. Each row holds information for a single animal, and the 
columns represent:
...
Since we already downloaded the data file that we will be using, we 
don't need to follow these directions at this time. However, in the future, 
you may need to access data from an internal server or somewhere on the 
internet. For example, if there is data being periodically written to a 
server, this command could be used in a script to automatically download the 
most recent version of a data file.

The R function `download.file()` would be used to download the CSV file that 
contains the survey data from Figshare. Inside the download.file command, 
the first entry is a character string with the source URL 
...
machine. You'll need to have a folder
on your machine called "data_raw" where you'll download the file. So this 
command downloads a file from figshare, names it "portal_data_joined.csv," 
and adds it to a preexisting folder named "data_raw."
...
surveys[[1]]
# Note: This will give a LOT of data!
```
