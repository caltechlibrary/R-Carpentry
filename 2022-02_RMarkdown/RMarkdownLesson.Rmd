...
`oajournals.bib` for you to work with.

References are handled in the `bibliography` section of the YAML header. Add
the following to the YAML in the document:
...
replace each reference with the appropriate tag, for example `@Vasilevsky_2017`.

The reference list will automatically added to the end of the document. You'll notice that
...
You can fix the metadata by grabbing a .bib file from Google Scholar, PubMed, or
Web of Science and copying the correct data into the `oajournals.bib` file.
The reference will be corrected when you knit the document again.
...
Let's add the child document at the end of the .Rmd file  You
